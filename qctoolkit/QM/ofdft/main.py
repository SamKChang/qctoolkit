...
import qctoolkit.analysis as qa
import qctoolkit.qtktools as qtk
ut = qtk.ut
np = qtk.np
copy = qtk.copy
...
class inp(GaussianBasisInput):
  def __init__(self, molecule, **kwargs):
    if not ht_found:
      ut.exit("horton module not found.")
    if not ps_found:
      ut.exit("pyscf module not found.")
    if not xc_found:
      ut.exit("libxc not found.")
