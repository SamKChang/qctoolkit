...
static PyObject * python_callback(PyObject * self, PyObject * args){
  int i_input, j_input; // filename string as input
  ...
  if(!PyArg_ParseTuple(args, "ii", &i_input, &j_input))
    return NULL;
  dims[0] = dim;
  dims[1] = dim;
  ...
  pythoncallback(input, dim, &coulombMatrix, dims);
...
static PyMethodDef pythonCallback[] = {
  {"python_callback", 
   python_callback, 
   METH_VARARGS, 
...
PyMODINIT_FUNC initpython_callback(void){
  Py_InitModule("python_callback", pythonCallback);
