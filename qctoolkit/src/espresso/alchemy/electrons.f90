...
SUBROUTINE electrons()
  ...
  DO idum=1,niter
     ...
     !
! KYSC start 20160315. Alchemy hack: Always write restart file
           print *, '    #### start writing restart_e file for alchemy...'
! KYSC    IF ( stopped_by_user .OR. .NOT. conv_elec ) THEN
! KYSC       conv_elec=.FALSE.
        IF ( .NOT. first) THEN
...
! KYSC       RETURN
! KYSC    END IF
           print *, '    #### finished writing restart_e'
! KYSC end 20160315
     !
...
SUBROUTINE electrons_scf ( printout )
  ...
  DO idum = 1, niter
     ...
     IF ( conv_elec .OR. MOD( iter, iprint ) == 0 ) THEN
        ...
        !
! KYSC star 20160315. Alchemy hack: always print ks energy
     ELSE
        CALL print_ks_energies()
! KYSC end 20160315
     END IF
