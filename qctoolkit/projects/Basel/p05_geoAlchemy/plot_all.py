...
legLoc_Et = [2,2,2,2,1,1,3,3,1,1,3,3]
legLoc_dE = [2,3,2,3,1,3,3,3,1,3,3,3]
name = ['p1h1','p2h1','p3h1']
scan = [1.0, 2.0]
ref_path = []
tar_path = []
  ...
  for j in range(3):
    for s in range(2):
      if i!=j:
        ...
        opt_path = []
        density_path = []
        print "processing (i, j, s) = (%d, %d, %d). filename: "\
          % (i+1, j+1, s+1)
        loadData(i,j,s)
        qal.PathData.loadCubeList(density_path)
        out, fig_name = pathPlot(i,j,s, no_show=True)
        print fig_name
        if out:
